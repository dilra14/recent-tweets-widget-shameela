...
// widget function
	class tp_widget_recent_tweets extends WP_Widget {
		...
		public function __construct() {
			$widget_ops = array( 'classname' => 'el-w-twitter'  );
			parent::__construct( 'shameela_twitter_widget', '&raquo; '.SHAMEELA_THEME_NAME.' Twitter' , 'shameela', $widget_ops);
		}
			...
			public function widget($args, $instance) {
				...
				if(!empty($instance['title'])){ $title = apply_filters( 'widget_title', $instance['title'] ); }
					...
					if(!empty($tp_twitter_plugin_tweets) && is_array($tp_twitter_plugin_tweets)){
						print '<div class="sidebarwidget" id="el-w-twitter">
						<div class="w-twitter-content">
							...
							foreach($tp_twitter_plugin_tweets as $tweet){					
								if(!empty($tweet['text'])){
									...
									print '<!-- twitter item 1 -->
										<div class="item">
											<!-- twitter line -->
											<div class="twitter-item">
												'.tp_convert_links($tweet['text']).'<a target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'"></a>
											</div>
											<!-- twitter intents -->
